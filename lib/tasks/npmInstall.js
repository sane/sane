...
var self = {
  ...
  npmInstall: function(packageNames, installPath, options) {
    ...
    console.log('2');
    var args = self.getArgs(packageNames, options.docker, options.installFolder, {
      save: options.save,
      saveDev: options.saveDev,
      saveExact: options.saveExact
    });
    console.log('3');
    var runOptions = self.getRunOptions(installPath, options.showOutput, options.docker, options.installFolder);

    console.log(mainCmd, args, runOptions);
