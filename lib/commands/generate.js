...
module.exports = function generate(blueprint, name, attributes, options, leek) {
  ...
  if (supportedBlueprints.indexOf(blueprint) > -1) {
    if (name !== undefined) {
      ...
      spawnSync(ember, emberOptions, { cwd: clientName, stdio: 'inherit' });
      spawnSync(sails, sailsOptions, { cwd: serverName, stdio: 'inherit' });
      spawnSync(sails, ['generate', 'controller', name], { cwd: serverName, stdio: 'inherit' });
    } else {
