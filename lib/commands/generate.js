...
module.exports = function generate(blueprint, name, attributes, options, leek) {
  ...
  if (supportedBlueprints.indexOf(blueprint) > -1) {
    if (name !== undefined) {
      ...
      spawn('ember', emberOptions, { cwd: clientName, stdio: 'inherit', env: process.env });
      spawn('sails', sailsOptions, { cwd: serverName, stdio: 'inherit', env: process.env });
      spawn('sails', ['generate', 'controller', name], { cwd: serverName, stdio: 'inherit', env: process.env });
    } else {
