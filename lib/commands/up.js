...
//Need to refactor to be able to use superspawn for windows compatibility
...
//TODO(markus): When using superspawn (for windows compatibilty) this needs to be rewritten
//See https://github.com/marcdiethelm/superspawn#api and https://github.com/marcdiethelm/superspawn#examples
...
module.exports = async function up(options, leek) {
  ...
  if (!options.skipSails) {
    if (options.docker && options.docker !== 'false') {
      ...
      var sailsExec = (process.platform === 'win32' ? 'sails.cmd' : 'sails');
      runAndOutput(sailsExec, ['lift'], { cwd: serverName, env: process.env }, serverPrintOptions);
    }
  }

  if (!options.skipEmber) {
    var emberExec = (process.platform === 'win32' ? 'ember.cmd' : 'ember');
    runAndOutput(emberExec, ['serve', '--proxy', `${emberProxy}:1337`, '--live-reload', options.liveReload],
