...
module.exports = async function newProject(name, options, leek) {
  ...
  if (!checkEnvironment.emberExists()) {
    ...
    var emberVersion;
    await spawn(ember, ['version'], { capture: ['stdout']})
      .then(function (result) {
        //  Get the first version number that the global ember spits out.
        emberVersion = result.stdout.match(semVer)[0];
  ...
  var sailsVersion;
  if (!options.docker) {
    await spawn(sails, ['version'], { capture: ['stdout'] })
      .then(function (result) {
        sailsVersion = result.stdout.toString();
      });
  } else {
    await spawn(dockerCompose, ['run', 'server', 'sails', 'version'], { capture: ['stdout'] })
      .then(function (result) {
        sailsVersion = result.stdout.toString();
      });
  }
