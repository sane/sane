...
var serverName = config.serverNames[0] || 'server';
var clientName = config.clientNames[0] || 'client'
...
module.exports = async function newProject(name, options, leek) {
  ...
  fs.writeFileSync(path.join('package.json'), prepareTemplate('package.json', { name: projectName }));
  ...
  var sailsPackages = ['sails-generate-ember-blueprints', 'lodash', 'pluralize', `sails-${options.database}`];
  await dockerExec(`npm i ${sailsPackages.join(' ')} --save`, options.docker, silent, options.skipNpm);
  await dockerExec('sails generate ember-blueprints', options, silent, options.skipNpm);
  ...
  if (!options.skipNpm) {
    ...
    console.log(`'sails-generate-ember-blueprints' (${chalk.underline('https://github.com/mphasize/sails-generate-ember-blueprints')})\n`);
  }
