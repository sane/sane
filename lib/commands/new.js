...
module.exports = function newProject(name, options, leek) {
  ...
  return regeneratorRuntime.async(function newProject$(context$1$0) {
    while (1) switch (context$1$0.prev = context$1$0.next) {
      ...
      case 7:
        context$1$0.next = 9;
        return spawn(npm, ['ls', '--global', '--json', '--depth=0', 'ember-cli'], { capture: ['stdout', 'stderr'] }).then(function (result) {
          if (options.verbose) {
      ...
      case 11:
        ...
        throw new Error('sane requires the latest docker-compose to be installed. ' + 'Check https://github.com/artificialio/sane/blob/master/README.md for more details.');
      ...
      case 64:
        ...
        context$1$0.next = 68;
        return spawn(sails, ['version'], { capture: ['stdout', 'stderr'] }).then(function (result) {
          if (options.verbose) {
            console.log('[spawn] stderr: ' + result.stderr.toString());
            console.log('[spawn] stdout: ' + result.stdout.toString());
          }
      ...
      case 71:
        context$1$0.next = 73;
        return spawn(dockerCompose, ['run', 'server', 'sails', 'version'], { capture: ['stdout', 'stderr'] }).then(function (result) {
          if (options.verbose) {
            console.log('[spawn] stderr: ' + result.stderr.toString());
            console.log('[spawn] stdout: ' + result.stdout.toString());
          }
