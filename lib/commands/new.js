...
var renameUnderscoreTemplate = require('../tasks/renameUnderscoreTemplate');
...
module.exports = async function newProject(name, options, leek) {
  ...
  var sailsPackages = ['sails-generate-ember-blueprints', 'lodash@^2.4.1',
  'morgan', 'pluralize', 'sails-hook-autoreload@~0.11.4', `sails-${options.database}`];
  ...
  for (var i = 0; i < templates.length; i++) {
    ...
    var templateOutPath = renameUnderscoreTemplate(templates[i]);
  ...
  try {
    ...
    console.log('Cleaning up Sails had an error. Don\'t worry at all, your project is still wonderful and shiny.')
    // console.log(err);
    // console.log('Still finishing your setup');
  }
  ...
  if (!options.skipNpm) {
    ...
    try {
      ...
      console.log('Had issues installing local sane-cli. Please run npm install manually.');
      console.log('See detailed error below:');
      console.log(err);
    }
  }
