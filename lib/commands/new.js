...
function prepareFigTemplate(name, option, user) {
...
module.exports = async function newProject(name, options, leek) {
  ...
  // Linux systems need the non-root user to be specified in fig.yml
  var prepareOptions = null;
  if (process.platform == 'linux') {
    prepareOptions = ['fig.yml', options.database, process.getuid()];
    var resetFigUser = true;
  } else {
    prepareOptions = ['fig.yml', options.database];
  }
  fs.writeFileSync(path.join('fig.yml'), prepareFigTemplate.apply(null, prepareOptions));
