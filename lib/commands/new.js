...
module.exports = function newProject(name, options) {
  ...
  execAbort.sync('mkdir ' + name,
    'Error: Creating a new folder failed. Check if the folder \'' + name + '\' already exists.', silent);
  ...
  execAbort.async('fig run server sails new .', 'Error: Creating a new sails project failed',
    ...
    'Sails container successfully created.', silent)
  .then(function(){
    ...
    async.series([
      function(callback){
        execAbort.asyncMulti('fig run server npm i sails-generate-ember-blueprints --save', callback, false, silent);
      },
      function(callback){
        execAbort.asyncMulti('fig run server npm i --save lodash', callback, false, silent);
      },
      function(callback){
        execAbort.asyncMulti('fig run server npm i --save pluralize', callback, false, silent);
      },
      function(callback){
        execAbort.asyncMulti('fig run server sails generate ember-blueprints', callback, false, silent);
      },
      function(callback){
        if (options.database === 'postgres') {
          execAbort.asyncMulti('fig run server npm i --save sails-postgresql', callback, false, silent);
        } else if (options.database !== 'disk') {
          execAbort.asyncMulti('fig run server npm i --save sails-' + options.database, callback, false, silent);
        } else {
          callback(null, '');
        }
      }],
      ...
      function(err, results){
        ...
        execAbort.async('ember new client', 'Error: Creating a new Ember Project failed',
          ...
          'Ember Project successfully created.', silent)
