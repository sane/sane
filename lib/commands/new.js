...
module.exports = async function newProject(name, options, leek) {
  ...
  // Linux systems need the non-root user to be specified in fig/docker-compose.yml
  var prepareOptions = null;
  if (process.platform === 'linux') {
    prepareOptions = [`${dockerCompose}.yml`, options.database, process.getuid()];
    var resetFigUser = true;
  } else {
    prepareOptions = [`${dockerCompose}.yml`, options.database];
  }
  fs.writeFileSync(path.join(`${dockerCompose}.yml`), prepareFigTemplate.apply(null, prepareOptions));
